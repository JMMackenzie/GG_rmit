...
# 5. Add in the mean data using stat_summary
plot <- plot +  stat_summary(fun.y=mean, colour="#333333", geom="point", 
               shape=18, size=3,show.legend = FALSE, position=position_dodge(width=0.75))
#
# 6. Make sure we are using the same text type as the paper we are writing
plot <- plot + theme(text = element_text(family = "Linux Libertine O", size=25))

# 7. Set the colours for the boxes we made manually
...
# 8. Move the legend if desired. Can have both positions such as "left", "right"
# or x/y coordinates
# Example 1:
...
# Save the PDF
ggsave(plot,file="mboxplot-mean.pdf",width=9,height=7, device=cairo_pdf)
embedFonts("mboxplot-mean.pdf",options='-c ".setpdfwrite <</NeverEmbed [ ]>> setdistillerparams" -dSubsetFonts=true -dCompressFonts=true -dCompatibilityLevel=1.4')
